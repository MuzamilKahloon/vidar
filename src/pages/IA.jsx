...
import React, { useState, useRef, useEffect } from 'react';
import { Send, TrendingUp, Activity, Target, X, Maximize2, Minimize2, Upload, FileText } from 'lucide-react';
...
const IA = () => {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      const uploadRes = await fetch('/context/learn_paths', {
        method: 'POST',
      ...
      const uploaded = files.map(file => ({
        ...
        file: file // Keep file reference for potential re-upload
      }));
      ...
      const uploadMsg = {
        id: Date.now(),
        type: 'bot',
        text: `Fichier(s) chargé(s) avec succès : ${uploaded.map(f => f.name).join(', ')}. Vous pouvez maintenant poser des questions à leur sujet.`,
        time: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
        analysis: null
      };
      ...
      const errorMsg = {
        id: Date.now(),
        type: 'bot',
        text: `Erreur lors du chargement du fichier: ${err.message}. Veuillez réessayer.`,
        time: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
        analysis: null
      };
  ...
  const handleSend = async () => {
    ...
    // Decide endpoint: use /chat/mixed if files uploaded, else /chat
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`API response error: ${res.status} - ${errorText}`);
      }
  ...
  const handleQuickAction = (prompt) => {
    setInputValue(prompt);
    // Auto-focus on textarea after setting value
    setTimeout(() => {
      if (textareaRef.current) {
      ...
      <div className="flex-1 bg-black flex flex-col">
        ...
        <div className="p-4 border-t border-gray-800 bg-black/50 backdrop-blur-sm">
          ...
          <div className="flex gap-3 items-end">
            ...
            <label className="p-3 rounded-2xl text-cyan-400 border border-cyan-500/30 hover:bg-cyan-500/10 cursor-pointer transition-all flex items-center justify-center">
              {isUploading ? (
